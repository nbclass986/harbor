...
- 属性条直接显示用户映射的 YAML 键名，例如内部 `due` 映射到 `deadline` 时显示并读写 `deadline`，不另设展示别名。八个不同名字段的映射测试 40/40 通过。
- 最新 UI 复测（2026-09-10）：1,000 条任务搜索／筛选／视图切换最高 P95 为 **54.6／34.6／59.3 ms**；5,000 条为 **61.1／33.7／75.8 ms**。每项 20 次，内存合成数据、真实渲染及两帧等待，不含磁盘初扫或进程冷启动。以下较早测试仅作为历史基线。
- 配色跟随 Obsidian 主题，结构和间距限定在 Harbor 内部。已测主题不代表全部第三方主题；自定义 CSS 仍可能产生冲突。

- 后续弹窗补测 33/33 通过：三主题下，新建任务／项目／资料的 400／720／1080 px 宽度，以及项目详情的 720／1080 px 宽度。修复部分按钮及项目控件缺少可见键盘焦点的问题。测试为组件宽度约束，不等同于手机或全部应用窗口尺寸。
